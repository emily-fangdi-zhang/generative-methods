...
let animations = [
  ...
  {
    title: "Construction",
    description: "construction site movement <p>INSIDE A TUNNEL!!</p>",
    ...
    draw(q, t){
      ...
      let x = (t * 100) % q.width;
      let y = 80;
      let a = q.width * (0.5 + 0.5 * Math.sin(t));
      let b = q.height * 0.5;
      let c = 100;
      let j = (t * 100) % q.width;
      let o = 220;
  ...
  // An example
  {
    title: "Raindrops",
    description:
      "what is the opposite of raindrops in The Giver? <p>colorful rain blobs</p>",
    isActive: true,
...

  {
    title: "Windshield",
    description:
      "POV: you are sitting inside the car and someone is wiping your windshield",
    isActive: true,
    ...
    draw(p, t) {
      ...
      let a = p.width * p.noise(t);
      let b = p.height * p.noise(t + 100);
      let c = 100;
...

  {
    ...
    draw(p, t) {
      ...
      // let radius = 100
      let radius = 10 * Math.sin(t);
      // let radius = 100*p.noise(t)
      // let radius = 100*p.noise(t*10)

      let x = radius + centerX*Math.cos(.4*this.theta)+20;
      let y = radius * Math.sin(this.theta)*300 + centerY;
      let r = 30;
    ...
    draw(p, t) {
      ...
      for (let i = 0; i < count; i++) {
        for (let j = 0; j < count; j++) {
          ...
          let hue = 700 * p.noise(x * noiseScale, Math.sin(t * noiseScale), t + 100);

          hue %= 40; // Wrap the hue around 306 degrees, P5 can't handle >360 hues
  ...
  {
    title: "Slash",
    description: "slashing through air",
    isActive: true,
    ...
    draw(p, t) {
      ...
      p.text("Time: " + t.toFixed(2), 3, 20);
      p.text("Loop: " + pct.toFixed(2), 1, 40);
      ...
      p.fill(0);
      p.circle(x, y, r);
      p.circle(x + 10, y + 20, r);
      p.circle(x*2, y*2, 10);
      p.circle(x*16, y*8, 10);
      
      for (var i = 0; i < 200; i++) {
        p.circle(x*i, y*i, 10);
        p.circle(x*10, y*i, 10);
        p.circle(x, y*i, r);
        p.circle(x*i, y, r);
        p.circle(x, y, r);
        p.circle(x+100, y+163, r);
      }
